...
其中， `p_age` ， `p_fair` ， `p_qos` 均为 0 到 1 范围内浮点数。各因子计算方法如下：
...
* `p_qos` ：优先级因子。现在，集群上提交的任务具有 `qos` 属性，可通过 `--qos` 参数**手动指定**，可选值为 `high` 与 `normal` ； `high` 级别任务的优先级因子为 1.0 ，`normal` 级别任务的优先级因子为 0.0
